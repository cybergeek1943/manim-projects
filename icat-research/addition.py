...
class Simplest(Scene):
    def construct(self):
        title = Text("Simple Example", font_size=36).to_corner(UL, 0.5)
        self.add(title)
        ...
        self.play(
            ReplacementTransform(VGroup(n1[0][3].copy(), n2[0][3].copy()), index_eq),
            index_eq.animate.set_color(GREEN)
        )
        self.wait()
        ...
        for i in (slice(7, 9), 6, slice(3, 6), 2, slice(0, 2)):            
            ...
            self.play(Transform(box, target_box), run_time=0.4)
            ...
            self.wait(0.2)
        target_box = SurroundingRectangle(final_sum, color=GREEN, buff=0.2)
        self.play(ReplacementTransform(box, target_box))
        self.wait()
        
        # final answer
        final_group = VGroup(final_sum, target_box)
        self.play(
            FadeOut(addition_group),
            final_group.animate.center()
        )
        self.play(
            final_group.animate.scale(2),
            Uncreate(target_box)
        )
        self.wait(4)
...
class Unpacking(Scene):
    def construct(self):
        title = Text("Alignment Unpacking", font_size=36).to_corner(UL, 0.5)
        self.add(title)
